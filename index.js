...
app.get('/characters/?nombre=nombre', async (req, res) => {
    res.send(await PersonajeService.getPersonajeByNombre(req.params.nombre))
})

app.get('/characters?edad=edad', async (req, res) => {
    res.send(await PersonajeService.getPersonajeByEdad(req.params.edad))
})

app.get('/characters?movies=idMovie', async (req, res) => {
    res.send(await PersonajeService.getPersonajeByMovieId(req.params.id))
})

//  Búsqueda de PeliSerie

app.get('/movies?name=nombre', async (req, res) => {
    res.send(await PeliYSerieService.getMovieByNombre(req.params.titulo))
})

app.get('movies/order/:order', async (req, res) => {
    res.send(await PeliYSerieService.getMovieByOrder(req.params.order))
})
